...
# Restaurant & Customer Multi-App Build Script
# ✅ Auto-updates Kotlin package/import + Android cleartext network policy
...
configure_cleartext_policy() {
  print_status "Configuring cleartext policy..."
...
EOF
  ...
  if ! grep -q "android:networkSecurityConfig" "$MANIFEST"; then
    ...
    print_success "Manifest updated for cleartext policy"
  else
    print_warning "Manifest already has cleartext config"
  fi
}

# --- Kotlin package updater ---
update_kotlin_package() {
  ...
  print_status "Updating MainActivity.kt for package: $APP_PACKAGE"

  # Find any existing MainActivity
  ...
  # Fix package + BuildConfig import
  sed -i "s/^package .*/package $APP_PACKAGE/" "$MAIN_ACTIVITY"
  sed -i "s|import .*BuildConfig|import $APP_PACKAGE.BuildConfig|" "$MAIN_ACTIVITY"

  # Sanity check
  head -n 2 "$MAIN_ACTIVITY" | grep -qE "^package [a-zA-Z0-9_.]+$" || print_error "Invalid package line in MainActivity.kt"

  print_success "Updated Kotlin package/import -> $APP_PACKAGE"
}
